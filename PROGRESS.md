...
- Valmis: 2026-06-30
- node-a:n agentti lähetti JSON-snapshotin WireGuard-tunnelin läpi (10.10.0.2 -> 10.10.0.1), mTLS:n suojaamana, node-b:n palvelimelle. Palvelin tunnisti agentin sertifikaatista (CN=node-agent) ja otti datan vastaan.
- Tämä oli koko projektin tähänastinen kohokohta. Macin rajallinen levytila/muisti hidasti etenemistä monta kertaa matkan varrella, mutta siitä huolimatta sain rakennettua toimivan yhteyden kahden virtuaalikoneen välille SSH:n kautta - tässä vaiheessa opin oikeasti, ei vain teoriassa, miten VPN, verkko ja WireGuard toimivat yhdessä käytännössä.
- Pari muuta opittua asiaa matkalta: SSH/scp saattoi hetkellisesti antaa "No route to host" -virheen UTM:n virtuaaliverkossa vaikka ping toimi normaalisti - kannattaa yrittää uudelleen ennen kuin alkaa debugata syvemmin. PATH-muutokset .profile-tiedostoon eivät periydy automaattisesti uuteen terminaali-istuntoon, vaikka käyttäjä on sama - source ~/.profile tarvitaan. Sertifikaatit siirsin scp:llä suoraan koneelta toiselle, koska certs/ on aina .gitignoressa eikä saa koskaan kulkea Git-historian kautta.
